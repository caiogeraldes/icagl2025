...
The variation of constructions with~\ref{gloss:attr} and
without~\ref{gloss:noattr} case attraction on arguments of infinitive clauses in
Ancient Greek remains largely unexplained.
...
A third group, that of structurally driven works, most often from the generative
or minimalist framework, has proposed structures that would allow the attraction
to occur, in just a handful of them proposing a specific structure that would
allow for the non\hyp{}attraction. Most recently,
\textcite{Sevdali2013,Sevdali2013b} (drawing part of their analysis from the
works of~\cite{Tantalou2003} and~\cite{Spyropoulos2005}) argued that case
attraction, or \emph{case transmission} as they call, is as a byproduct of the
syntactic marking of $[{Person}]$ in a otherwise \emph{person}-deficient clause,
i.e.\ infinitive clauses. In other words, they assume the existence of a
deficient Person head projected over the infinitive verb with a caseless
\textsc{pro}, which therefore would probe for case until if finds the oblique
object of the matrix clause, and finally provides case for its predicate when it
probes for case. The existence of~\ref{gloss:noattr} is explained by means of an
optional accusative \emph{pro} acting as subject for the infinitival clause and
which the author argues that is available in Ancient Greek when the subject of
infinitives refers to an element ``prominent in the discourse but not present in
the clause that immediately contains the infinitive''
(\cite[42--3]{Sevdali2013}). Similarly to the ``emphasis'' explanation, however,
the ``discourse prominence'' does not comes with empirical evidence or clear
definition.
